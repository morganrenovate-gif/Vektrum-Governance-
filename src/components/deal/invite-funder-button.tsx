  ...
  const ExpiryRow = ({ expiresAt, onNew }: { expiresAt: string; onNew: () => void }) => (
    <div className="flex items-center justify-between text-xs text-vektrum-faint">
      ...
      <div className="flex gap-3">
        <button onClick={onNew} className="flex items-center gap-1 hover:text-vektrum-text transition-colors">
          <RefreshCw className="h-3 w-3" />New link
        </button>
        <button onClick={reset} className="hover:text-vektrum-text transition-colors">Done</button>
      </div>
    </div>
  )
  ...
  if (state.phase === 'error') {
    return (
      <div className="space-y-2">
        ...
        <button onClick={reset} className="text-xs text-vektrum-muted hover:text-vektrum-text transition-colors">
          Try again
        </button>
      </div>
    )
