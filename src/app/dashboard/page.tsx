...
function EmptyDeals({ role }: { role: string }) {
  return (
    <div className="text-center py-16 rounded-2xl border border-dashed border-white/[0.08]">
      <FolderOpen size={40} className="mx-auto text-white/25 mb-4" aria-hidden="true" />
      <p className="text-[15px] font-semibold text-white mb-1">No deals yet</p>
...
// ─── Section header — consistent with page standard ───────────────────────────
function SectionLabel({ label, color = 'text-white/40' }: { label: string; color?: string }) {
  return (
    <div className="mb-5 flex items-center gap-3">
      <div className="h-px w-5 bg-vektrum-blue" />
      <p className={`text-[11px] font-semibold uppercase tracking-[0.12em] ${color}`}>
        {label}
      </p>
    </div>
  )
  ...
  if (!profile) {
    return (
      <div className="min-h-screen bg-[#0D1B2A] flex items-center justify-center">
        {/* VISUAL: replaced light-mode Card tokens with explicit dark surface */}
        <div
          className="rounded-2xl border border-white/[0.08] bg-[#111827] px-8 py-7 max-w-sm w-full"
          ...
          <div className="max-w-6xl mx-auto px-6 sm:px-8 lg:px-12 py-12 sm:py-16 space-y-8">
            ...
            <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div>
                ...
                {/* VISUAL: h1 scale corrected from text-[2.25rem] → text-[2.75rem] */}
                <h1 className="font-display text-[2.75rem] font-bold tracking-[-0.04em] text-white leading-[1.05]">
            ...
            {(() => {
              ...
              return (
                // VISUAL: replaced conflicting inline style + className border with clean
                // dark card surface matching the established card standard
                <div
                  className={`rounded-2xl border border-white/[0.08] bg-[#111827] px-5 py-4 flex items-center justify-between border-l-4 ${accentColor}`}
                  style={{ boxShadow: '0 8px 32px rgba(0,0,0,0.35), 0 0 0 1px rgba(255,255,255,0.03)' }}
                >
            ...
            {/* Stripe setup banner */}
            {!profile.stripe_account_id && (
              // VISUAL: replaced light-mode amber tokens with dark-context treatment
              <div
                className="flex items-start gap-3 rounded-2xl border border-white/[0.08] bg-[#111827] px-5 py-4"
            ...
            {/* VISUAL: replaced border-l-4 accent header with consistent card header treatment */}
            <div
              className="rounded-2xl border border-white/[0.08] overflow-hidden"
              style={{ boxShadow: '0 8px 32px rgba(0,0,0,0.35), 0 0 0 1px rgba(255,255,255,0.03)' }}
            >
              <div className="flex items-center gap-3 px-5 py-4 border-b border-white/[0.06] bg-[#111827]">
                <div className="h-px w-5 bg-vektrum-blue" />
            ...
            {/* Deals */}
            <section>
              {/* VISUAL: replaced raw text label with accent-line section header */}
              <SectionLabel label="Your Deals" />
              {deals.length === 0 ? (
        ...
        <div className="min-h-screen bg-[#0D1B2A]">
          <div className="max-w-6xl mx-auto px-6 sm:px-8 lg:px-12 py-12 sm:py-16 space-y-8">
            ...
            {actionRequiredSorted.length > 0 && (
              <section>
                {/* VISUAL: replaced raw amber text label with accent-line section header */}
                <SectionLabel label={`Action Queue (${actionRequiredSorted.length})`} color="text-vektrum-amber" />
                <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                  {actionRequiredSorted.map((deal) => (
                    <div key={deal.id} className="space-y-3">
                      <DealCard deal={deal} />
                      {/* VISUAL: replaced light-mode surface tokens with dark card standard */}
                      <div
                        className="rounded-2xl border border-white/[0.08] bg-[#111827] px-4 py-4"
                        style={{ boxShadow: '0 4px 16px rgba(0,0,0,0.25)' }}
                      >
            ...
            <section>
              {/* VISUAL: replaced raw text label with accent-line section header */}
              <SectionLabel label="Funded Deals" />
              {funded.length === 0 ? (
