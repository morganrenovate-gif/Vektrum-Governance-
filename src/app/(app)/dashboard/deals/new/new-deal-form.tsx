...
export type DealFormRole = "contractor" | "funder" | "admin";

interface NewDealFormProps {
  /**
   * The authenticated user's role, resolved server-side before this component
   * mounts. Passed as a prop so the initial render is immediately correct —
   * no useEffect role-fetch, no null initial state, no role-flash.
   */
  role: DealFormRole;
}
...
export function NewDealForm({ role }: NewDealFormProps) {
  const router = useRouter();

  // role is a server-resolved prop — no async, no null initial state, no flash.
  ...
  // Fetch execution-rail partners for admin users.
  // This is the only async effect — role is already known from the prop.
  useEffect(() => {
    if (!isAdmin) return;
